...
% Assign random probabilities of interaction to each node
nodeProbabilities  = normrnd(0.1,0.5,length(PopNodes));
...
% Plot histograms for male and female node degrees on the same graph
figure;
hold on;
histogram(nodeDegrees(maleNodes), 'FaceColor', [0 0 1]);
histogram(nodeDegrees(femaleNodes), 'FaceColor', [1 0 0]);
title('Node Degree Distributions by Gender', 'Interpreter', 'latex');
xlabel('Degree', 'Interpreter', 'latex');
ylabel('Frequency', 'Interpreter', 'latex');
legend({'Males', 'Females'}, 'Interpreter', 'latex');
hold off;

% Save the histogram figure
saveas(gcf, 'node_degrees_histogram.pdf');
